#!/usr/bin/env python3
"""Benchmark fused_ce: costmodel vs brute-force autotune.

Source: mojo_opset/backends/ttx/kernels/npu/fused_linear_cross_entropy.py
"""
...
def main():
    ...
    from kernels.mojo_opset.a5 import fused_linear_cross_entropy as mod_a
    common.reload_kernel_module(mod_a)
    common.warmup_cache()
    a_elapsed, a_timings, a_preds, a_best, a_cm, a_hw = common.run_autotune_pass(
        mod_a, "_cross_entropy_kernel",
        lambda: mod_a.fused_linear_cross_entropy_fwd_impl(hidden, weight, target),
    ...
    from kernels.mojo_opset.a5 import fused_linear_cross_entropy as mod_b
    common.reload_kernel_module(mod_b)
    common.warmup_cache()
    b_elapsed, b_timings, _, b_best, _, b_hw = common.run_autotune_pass(
        mod_b, "_cross_entropy_kernel",
        lambda: mod_a.fused_linear_cross_entropy_fwd_impl(hidden, weight, target),
        tuner_fallbacks=None,
    )
    ...
    with open(os.path.join(os.path.dirname(__file__), "results_fused_ce.json"), "w") as f:
        json.dump({"operator": "fused_ce", "top_k": TOP_K, "speedup": b_elapsed / a_elapsed if a_elapsed else float("inf")}, f, indent=2)
    print("Results saved")
