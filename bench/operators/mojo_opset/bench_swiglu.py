#!/usr/bin/env python3
"""Benchmark swiglu: costmodel vs brute-force autotune.

Source: mojo_opset/backends/ttx/kernels/npu/swiglu.py
"""
...
def main():
    # ---- inputs ----
    a = torch.randn(4, 1024, 4096, device='npu', dtype=torch.float16)
    b = torch.randn(4, 1024, 4096, device='npu', dtype=torch.float16)

    # ---- Pipeline A ----
    common.setup_costmodel_env(True, TOP_K)
    from kernels.mojo_opset.a5 import swiglu as mod_a
    common.reload_kernel_module(mod_a)
    common.warmup_cache()
    a_elapsed, a_timings, a_preds, a_best, a_cm, a_hw = common.run_autotune_pass(
        mod_a, "_swiglu_fwd_kernel",
        lambda: mod_a.swiglu_fwd_impl(a, b),
    ...
    with open(os.path.join(os.path.dirname(__file__), "results_swiglu.json"), "w") as f:
        json.dump({"operator": "swiglu", "top_k": TOP_K, "speedup": b_elapsed / a_elapsed if a_elapsed else float("inf")}, f, indent=2)
    print("Results saved")
