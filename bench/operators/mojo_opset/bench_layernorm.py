#!/usr/bin/env python3
"""Benchmark layernorm: costmodel vs brute-force autotune.

Source: mojo_opset/backends/ttx/kernels/npu/layernorm.py
"""
...
def main():
    ...
    from kernels.mojo_opset.a5 import layernorm as mod_a
    common.reload_kernel_module(mod_a)
    common.warmup_cache()
    a_elapsed, a_timings, a_preds, a_best, a_cm, a_hw = common.run_autotune_pass(
        mod_a, "_layernorm_fwd_kernel",
        lambda: mod_a.layernorm_infer_impl(x, w, b, 1e-6),
    ...
    with open(os.path.join(os.path.dirname(__file__), "results_layernorm.json"), "w") as f:
        json.dump({"operator": "layernorm", "top_k": TOP_K, "speedup": b_elapsed / a_elapsed if a_elapsed else float("inf")}, f, indent=2)
    print("Results saved")
