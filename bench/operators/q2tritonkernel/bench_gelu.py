#!/usr/bin/env python3
"""Benchmark GELU: costmodel-predicted vs brute-force autotune.

Kernel source (copied as-is, imports only adjusted):
    kernels/gelu.py  ←  Q2TritonKernel/src/kernels/gelu.py

Usage::

    TRITON_COSTMODEL_TOP_K=5 python bench/operators/bench_gelu.py
"""
...
def main() -> None:
    ...
    # Prepare input
    # ------------------------------------------------------------------
    shape = (4, 1024, 4096)  # B, T, H — realistic transformer hidden states
    ...
    from kernels.q2tritonkernel import gelu as gelu_a
    common.reload_kernel_module(gelu_a)
    common.warmup_cache()
    ...
    from kernels.q2tritonkernel import gelu as gelu_b
    common.reload_kernel_module(gelu_b)
    common.warmup_cache()
    ...
    out_path = os.path.join(os.path.dirname(__file__), "results_gelu.json")
