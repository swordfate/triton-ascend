...
cd ${CUR_PATH}
rm -rf Q2TritonKernel
source /data/set_proxy.bash
git config --global http.sslverify false
git clone https://zhiliangtang0727:hpyNCursTuTNMY1sq4bNxNTw@gitcode.com/TritonAscendTest/Q2TritonKernel.git
PYTHONPATH=${CUR_PATH}/Q2TritonKernel/src/kernels:${CUR_PATH}/Q2TritonKernel/src:${CUR_PATH}/Q2TritonKernel:$PYTHONPATH

TESTCASE_DIR=${CUR_PATH}/Q2TritonKernel/tests/fla/
mkdir -p ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/low_precision
mkdir -p ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/high_precision
\cp /data/PytorchFile/fla_operator_cases/wy_fast_gated_delta_bwd_pt/baseine_data_torch_npu_low_precision.tar.gz ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/low_precision/
\cp /data/PytorchFile/fla_operator_cases/wy_fast_gated_delta_bwd_pt/baseline_data_high_prec.tar.gz ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/high_precision/
cd ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/high_precision/
tar -zxvf baseline_data_high_prec.tar.gz
cd ${CUR_PATH}/Q2TritonKernel/tests/fla/data/wy_fast_gated_delta_bwd/low_precision/
tar -zxvf baseine_data_torch_npu_low_precision.tar.gz


LIST_FILE=${CUR_PATH}/testcase.txt
...
#            echo "[DEBUG] $$(date +%F_%T) NPU$npu_id  PWD=$(pwd)  CMD=pytest -sv $line -n 8 --junitxml=$test_xml"
            ...
            TRITON_CACHE_DIR=$(pwd)/cache/${line} ASCEND_RT_VISIBLE_DEVICES=$npu_id pytest -sv  --reruns 1 --reruns-delay 60 --only-rerun "Failed to start the device|out of memory" -sv "$line" --junitxml="$test_xml" > "$test_log" 2>&1
            echo "$npu_id" >&9
        } &
...
process_perf_data "${CUR_PATH}/megablocks_gpu_perf.csv" "${CUR_PATH}"

\cp *operator_perf.csv ${OP_LOG_DIR}/

find ${CUR_PATH}/Q2TritonKernel/ -type f -size +1G -exec rm {} +
