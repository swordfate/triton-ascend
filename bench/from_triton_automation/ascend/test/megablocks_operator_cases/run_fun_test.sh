...
TESTCASE_DIR=${CUR_PATH}/Q2TritonKernel/tests/megablocks
...
#            echo "[DEBUG] $$(date +%F_%T) NPU$npu_id  PWD=$(pwd)  CMD=pytest -sv $line -n 8 --junitxml=$test_xml"
            ...
            TRITON_DEBUG=1 TRITON_CACHE_DIR=$(pwd)/cache/${line} ASCEND_RT_VISIBLE_DEVICES=$npu_id pytest -sv  --reruns 1 --reruns-delay 60 --only-rerun "Failed to start the device|out of memory" -sv "$line" --junitxml="$test_xml" > "$test_log" 2>&1
            echo "$npu_id" >&9
        } &
...
\cp npu_performance_summary.csv ${OP_LOG_DIR}/megablocks_opperf.csv
