...
git clone -b master https://zhiliangtang0727:hpyNCursTuTNMY1sq4bNxNTw@gitcode.com/TritonAscendTest/mojo_opset
...
LIST_FILE=${CUR_PATH}/not_pass_testcase.txt
...
#            echo "[DEBUG] $$(date +%F_%T) NPU$npu_id  PWD=$(pwd)  CMD=pytest -sv $line -n 8 --junitxml=$test_xml"
            TRITON_CACHE_DIR=$(pwd)/cache/${line} ASCEND_RT_VISIBLE_DEVICES=$npu_id pytest -sv "$line" -n 8 --junitxml="$test_xml" > "$test_log" 2>&1
            echo "$npu_id" >&9
        } &
