...
cd ./mojo_opset
#cp -r /data/PytorchFile/Mojo_opset/dllm ./tests/specific_shape_perf_acc/test_data/
pip install -e .
...
#            echo "[DEBUG] $$(date +%F_%T) NPU$npu_id  PWD=$(pwd)  CMD=pytest -sv $line -n 8 --junitxml=$test_xml"
            ...
            TRITON_DEBUG=1 TRITON_CACHE_DIR=$(pwd)/cache/${line} ASCEND_RT_VISIBLE_DEVICES=$npu_id MOJO_Backend=ttx pytest -sv --reruns 1 --reruns-delay 60 --only-rerun "Failed to start the device|out of memory" -sv "$line" --junitxml="$test_xml" > "$test_log" 2>&1
            echo "$npu_id" >&9
        } &
...
# run gene case
TEST_generalization="${CUR_PATH}/mojo_opset"
