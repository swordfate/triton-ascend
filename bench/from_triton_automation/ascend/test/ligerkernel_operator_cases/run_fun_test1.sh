...
#        done
    ...
    while IFS= read -r test_name && IFS= read -r path; do
        ...
        {
            TRITON_CACHE_DIR=$(pwd)/cache/${file} ASCEND_RT_VISIBLE_DEVICES=$npu_id timeout 2400 pytest -sv "$path" -n 1 --junitxml="${OP_LOG_DIR}/${file%.py}_results.xml" > $test_log 2>&1
            echo "$npu_id" >&9
        } &
