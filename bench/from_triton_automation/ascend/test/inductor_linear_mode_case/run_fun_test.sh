...
cd ${CUR_PATH}
rm -rf inductor_linear_mode_case
source /data/set_proxy.bash
git config --global http.sslverify false
git clone https://cyd_199652:aWWPrcXuAPHYwrskHx5AFBt6@gitcode.com/TritonAscendTest/TritonAutomation.git inductor_linear_mode_case

TESTCASE_DIR=${CUR_PATH}/inductor_linear_mode_case/
...
#            echo "[DEBUG] $$(date +%F_%T) NPU$npu_id  PWD=$(pwd)  CMD=pytest -sv $line -n 8 --junitxml=$test_xml"
            ...
            # 【核心修改点】: 去掉了 --only-rerun 限制，并将重跑次数 `--reruns` 改为 3 次，延迟 `--reruns-delay` 改为 30 秒。
            # 这样无论遇到什么原因导致用例失败，都会自动进行重跑。
            \cp ${CUR_PATH}/../script/template.xml $test_xml
            TRITON_CACHE_DIR=$(pwd)/cache/${line} ASCEND_RT_VISIBLE_DEVICES=$npu_id timeout 3600 pytest --reruns 3 --reruns-delay 30 -sv "$line" -n 8 --junitxml="$test_xml" > "$test_log" 2>&1
