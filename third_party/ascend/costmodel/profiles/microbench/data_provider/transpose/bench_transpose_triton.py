...
@triton.jit
def tile_kernel(x_ptr, out_ptr, ITERS, MODE: tl.constexpr,
...
def measure(mode, op, ops, grid, bm, bn, reps, sf):
    x, out = _tensor_for_kernel(grid, bm, bn)
    opts = _opts(mode, grid, sf)

    def launch():
        tile_kernel[(grid,)](
            x, out, ITERS=ops, MODE=op, BM=bm, BN=bn, **opts
        )
...
def main():
    ...
    ap.add_argument("--ops", nargs="+", default=["add", "trans"],
                    choices=["add", "trans"])
    ap.add_argument("--ops-list", type=int, nargs="+",
    ...
    for mode in args.modes:
        for op_name in args.ops:
            ...
            for ops in args.ops_list:
                try:
                    ms = measure(
                        mode, op, ops, args.grid, args.bm, args.bn,
