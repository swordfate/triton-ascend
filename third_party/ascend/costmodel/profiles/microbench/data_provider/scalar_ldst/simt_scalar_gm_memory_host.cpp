// Host driver for simt_scalar_gm_memory_random.cce.
...
int main() {
  ...
  rtSetDevice(0);
  char *binary = nullptr;
  void *handle = reg("simt_scalar_gm_memory.o", &binary);
  ...
  runK(fn, stream, dout, gm, 1, 4, 16, 0, 1);
  printf("SIMT uniform random-spaced scalar GM memory probe\n");
  printf("== per-warp uniform random (same address across lanes) ==\n");
  printf("warps,ops,load_cycles,load_warp_instr_per_cycle,store_cycles,store_warp_instr_per_cycle\n");
  for (int warps : {1, 2, 4, 8, 16, 32}) {
    for (int ops : {1, 2, 4, 8}) {
      double loadCycles = cyclesPerIter(fn, stream, dout, gm, warps, 0, ops);
      double storeCycles = cyclesPerIter(fn, stream, dout, gm, warps, 1, ops);
      double loadRate = (double)warps * ops / loadCycles;
      double storeRate = (double)warps * ops / storeCycles;
      printf("%d,%d,%.6f,%.6f,%.6f,%.6f\n", warps, ops, loadCycles,
             loadRate, storeCycles, storeRate);
    }
  }
