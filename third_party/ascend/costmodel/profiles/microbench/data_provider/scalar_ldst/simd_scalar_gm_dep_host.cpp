// Host driver for simd_scalar_gm_dep.cce.
...
static long long minimum(const char *fn, rtStream_t stream, void *dout,
                         void *gm, int K, int iters, int mode,
                         int ops) {
...
int main() {
  ...
  rtSetDevice(0);
  char *binary = nullptr;
  void *handle = reg("simd_scalar_gm_dep.o", &binary);
  ...
  // Fill a random 64B-spaced linked-list path.
  const int kNodes = 1 << 20;
  vector<int> next(kNodes);
  ...
  printf("SIMD MainScalar scalar GM dependency probe\n");
  printf("ops,independent_cycles,dependent_cycles,extra_per_edge_cycles\n");
  for (int ops : {1, 2, 4, 8}) {
    double indep = cyclesPerIter(fn, stream, dout, gm, 0, ops);
    double dep = cyclesPerIter(fn, stream, dout, gm, 1, ops);
    printf("%d,%.6f,%.6f,%.6f\n", ops, indep, dep,
