...
def simd_loads(dump):
    ...
    for line in instr.splitlines():
        ...
        if iid not in issue or iid not in retire:
            continue
        rows.append({"id": iid, "issue": issue[iid], "retire": retire[iid],
                     "win": retire[iid] - issue[iid], "name": name, "addr": addr})
...
def simt_loads(dump):
    ...
    for line in read(dump / "core0.veccore0.rvec.simt.dc.dump").splitlines():
        m = re.search(
            r"\[(\d+)\]\[TagRam\] C6,.*addr:(0x[0-9a-fA-F]+), size:(\d+),.*isaId:(\d+)", line
        )
        if m:
    ...
    for iid, ev in events.items():
        ...
        if not d or d["size"] >= 128:  # scalar only; exclude vector tile LDG
            continue
        rows.append({"id": iid, "issue": ev["issue"], "retire": ev["retire"],
                     "win": ev["retire"] - ev["issue"], "addr": d["addr"],
                     "size": d["size"]})
    return sorted(rows, key=lambda r: r["issue"])
...
def simd_store(dump):
    ...
    for iid, (t, name, pc) in sorted(pushes.items(), key=lambda kv: kv[1][0]):
        if name != "MOV_SRC_TO_DST_ALIGNv2" or iid not in retires:
            continue
        rows.append({"id": iid, "issue": t, "retire": retires[iid],
                     "win": retires[iid] - t, "name": name, "pc": pc})
    return rows
...
def simt_store(dump):
    ...
    for iid, rec in events.items():
        if "ISSUE_INSTR" in rec and "RETIRE_INSTR" in rec:
            rows.append({"id": iid, "issue": rec["ISSUE_INSTR"], "retire": rec["RETIRE_INSTR"],
                         "win": rec["RETIRE_INSTR"] - rec["ISSUE_INSTR"], "name": "SIMT_STG"})
    return sorted(rows, key=lambda r: r["issue"])
    ...
    for st in d["stage_model"]["logical_stages"]:
        ...
        for impl in st["implementations"]:
            ...
            if im["mode"] == mode and im["superblock_factor"] == 1:
                ...
                cycles = rc.get("load_per_iteration", 0.0) if model == "scalar_load" else rc.get("store_per_iteration", 0.0)
                break
        if cycles is None:
            continue
        item = {"id": st["id"], "cycles": float(cycles),
...
def main():
    ...
    ap.add_argument("--base", required=True,
                    help="CAModel eval dir containing out/<kernel>_<mode>.run.log")
