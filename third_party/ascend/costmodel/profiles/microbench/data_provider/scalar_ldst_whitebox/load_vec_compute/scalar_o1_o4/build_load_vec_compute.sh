#!/usr/bin/env bash
set -eo pipefail
# Build the scalar-load + vector-compute CAModel probes.
cd "$(dirname "$0")"
...
ccec -c -std=c++17 -O2 --cce-aicore-only --cce-aicore-arch=dav-c310 \
  -I"$INC" load_vec_compute.cce -o load_vec_compute.o

g++ -O2 load_vec_compute_host.cpp -o load_vec_compute_host \
  -I"$ASCEND_TOOLKIT_HOME/x86_64-linux/pkg_inc" \
  -I"$ASCEND_TOOLKIT_HOME/include" \
  -L"$ASCEND_TOOLKIT_HOME/lib64" -lruntime -lascendcl

echo "BUILD DONE: $(pwd)/load_vec_compute.o $(pwd)/load_vec_compute_host"
