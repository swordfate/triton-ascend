...
struct Args { void* out; void* gm; int base; int reps; };
...
static void* reg(const char* bin, char** buf) {
  ...
  *buf = readBin(bin, &sz);
  ...
  b.version = 0;
  void* h = nullptr;
  if (rtDevBinaryRegister(&b, &h) != 0) { fprintf(stderr, "register bin failed\n"); exit(1); }
  return h;
}
...
static long long runK(void* fn, rtStream_t stream, void* dout, void* gm,
  ...
  if (ret != 0) { fprintf(stderr, "launch failed ret=%d\n", ret); exit(1); }
  rtStreamSynchronize(stream);
  long long cycles = 0;
  rtMemcpy(&cycles, sizeof(cycles), dout, sizeof(cycles), RT_MEMCPY_DEVICE_TO_HOST);
  return cycles;
}
...
int main(int argc, char** argv) {
  ...
  vector<string> funcs;
  { ifstream f(listfile); string line; while (getline(f, line)) if (!line.empty()) funcs.push_back(line); }
  if (funcs.empty()) { fprintf(stderr, "empty list\n"); return 1; }

  if (aclInit(nullptr) != 0) { fprintf(stderr, "aclInit failed\n"); return 1; }
  if (rtSetDevice(0) != 0) { fprintf(stderr, "rtSetDevice failed\n"); return 1; }

  char* buf = nullptr;
  void* handle = reg(obj, &buf);
  for (const auto& name : funcs) {
    if (rtFunctionRegister(handle, name.c_str(), name.c_str(), (void*)name.c_str(), 0) != 0) {
      fprintf(stderr, "rtFunctionRegister failed: %s\n", name.c_str()); return 1;
    }
  }

  rtStream_t stream; rtStreamCreate(&stream, 0);
  void* dout = nullptr; void* gm = nullptr;
  if (rtMalloc(&dout, 64, RT_MEMORY_HBM, 0) != 0) { fprintf(stderr, "malloc out failed\n"); return 1; }
  if (rtMalloc(&gm, GM_BYTES, RT_MEMORY_HBM, 0) != 0) { fprintf(stderr, "malloc gm failed\n"); return 1; }
  ...
  for (size_t fi = 0; fi < funcs.size(); ++fi) {
    ...
    long long b1 = runBest((void*)name, stream, dout, gm, I1, R, base_i1, REGION_STEP);
    long long b2 = runBest((void*)name, stream, dout, gm, I2, R, base_i2, REGION_STEP);
    double slope = double(b2 - b1) / double(I2 - I1);
    printf("%s,%.3f,%lld,%lld\n", name, slope, b1, b2);
    fflush(stdout);
  }

  rtFree(gm); rtFree(dout); rtStreamDestroy(stream);
  rtDeviceReset(0); aclFinalize(); delete[] buf;
  return 0;
}
