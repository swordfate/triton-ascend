...
def parse_simt(root: pathlib.Path):
    ...
    for op in ops:
        for line in dc:
            ...
            if m and int(m.group(3)) == op["id"]:
                ...
                break
        for line in ub:
            m = re.search(r"\[(\d+)\] \[UBITF_RCEV_REQ\] \[DC_BPSQ_WR\] SIMT_STG .*addr (0x[0-9a-fA-F]+).*thread_count (\d+)", line)
    ...
    writes = []
    for line in bw:
        m = re.search(r"\[(\d+)\]\s*:?\s*recv_simt_wr_cmd, port: (vdcache\d+), addr: (0x[0-9a-fA-F]+).*gid:\s*(\d+)", line)
        if m and m.group(3).lower() in TARGET_ADDRS:
            writes.append({"time": int(m.group(1)), "port": m.group(2), "addr": m.group(3).lower(), "gid": int(m.group(4))})
    return {
        "case": root.name,
        "window": (max(o["retire"] for o in ops) - min(o["issue"] for o in ops)) if ops else None,
        "ops": ops,
