...
def parse_simd(root: pathlib.Path):
    ...
    for line in sq:
        ...
        m = re.search(r"\[info\]\s+(\d+) \[RETIRE INSTR\] instr.name=ST_XD_XN_IMM\s+instr.pc=(0x[0-9a-fA-F]+)\s+instr.id=(\d+)", line)
        if m and m.group(2).lower() == st["pc"] and int(m.group(3)) == st["id"]:
    ...
    if "wr_id" in ev:
        for line in bwif:
            m = re.search(r"\[info\] \[(\d+)\]: (recv_su_wr_cmd|send_aw_cmd|send_wr_cmd_rsp|recv_wr_data|write_store_buf|recv_wack|send_wr_data|recv_brsp|send_data_rsp).*gid:\s*(\d+)", line)
...
def parse_simt(root: pathlib.Path):
    ...
    if gid is not None:
        for line in bwif:
            m = re.search(r"\[(\d+)\]\s*:?\s*(send_aw_cmd|send_wr_cmd_rsp|recv_wr_data|write_store_buf|recv_wack|send_wr_data|recv_brsp|send_data_rsp).*gid:\s*(\d+)", line)
