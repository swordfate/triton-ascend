...
def parse_simd(root: pathlib.Path):
    ...
    return {
        ...
        "summary": {
            ...
            "biu_line_count": len({e["addr"] for e in events if e["event"] == "biu_req" and e["addr"] in ld_addrs}),
        },
    }
...
def parse_simt(root: pathlib.Path):
    ...
    for line in biu_lines:
        m = re.search(
            r"\[info\]\s+(\d+): send_rd_cmd, port: (\w+).*addr: (0x[0-9a-fA-F]+).*gid: (\d+)", line
        )
        if m:
            sends.append(
                {
    ...
    for i in order:
        ...
        cand = [
            s
            for s in sends
            if s.get("port", "").startswith("vicache0")
            and s["gid"] not in used_gids
            and s["addr"] == tag.get("addr")
            and s["time"] >= tag.get("tag", 0)
        ]
    ...
    return {
        ...
        "ops": ops,
        "summary": {
            "line_count": len({o["addr"] for o in ops if o.get("addr")}),
