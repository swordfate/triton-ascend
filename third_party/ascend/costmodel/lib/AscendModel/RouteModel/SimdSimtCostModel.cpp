...
struct CandidateProfile {
  ...
  /// Kernel-only fixed prologue (in-kernel dispatch/pipe setup visible in
  /// profiler duration), fitted from profiler-mode 5-case measurements.
  double simdFixedOverheadCycles = 0.0;
  ...
  double simtScanElementsPerCycle = 0.0;
  ...
  /// Kernel-only fixed prologue (SIMT runtime-loop setup inside the kernel),
  /// fitted from profiler-mode small-kernel measurements (~3.3k cycles).
  double simtFixedOverheadCycles = 0.0;
...
loadCandidateProfile(llvm::StringRef requestedPath) {
  ...
  if (simd) {
    ...
    profile.simdFixedOverheadCycles =
        reader.optionalNumber(*simd, "fixed_overhead_cycles", 0.0);
  }
  ...
  if (simt) {
    ...
    if (const auto *scan = reader.object(*simt, "scan", "simt")) {
      ...
      profile.simtScanElementsPerCycle = reader.optionalNumber(
          *scan, "elements_per_system_cycle", 0.0);
    }
    ...
    profile.simtFixedOverheadCycles =
        reader.optionalNumber(*simt, "fixed_overhead_cycles", 0.0);
    if (const auto *dot = reader.object(*simt, "dot", "simt")) {
...
mlir::ascend::estimateSimdSimtCandidates(
  ...
  auto simtScanCost = [&](int64_t elements) {
    double cycles = profile.simtScanFixedCycles;
    if (profile.simtScanElementsPerCycle > 0.0)
      cycles += static_cast<double>(elements) /
                profile.simtScanElementsPerCycle;
    return cycles;
  };
  if (scanElements > 0 && profile.simdScanElementsPerCycle > 0.0) {
    ...
    report.breakdown.simdComputeCycles +=
        report.breakdown.simdScanCycles;
    report.breakdown.simtScanCycles = simtScanCost(scanElements);
  }
  ...
  if (anchorScanElements > 0)
    report.breakdown.mixedSimtAnchorScanCycles =
        simtScanCost(anchorScanElements);
  ...
  // fixed per-invocation costs are deliberately not scored.  The remaining
  // fixed_overhead_cycles terms are the in-kernel prologues visible in
  // profiler duration (SIMD pipe setup ~2k, SIMT runtime-loop setup ~3.3k).
