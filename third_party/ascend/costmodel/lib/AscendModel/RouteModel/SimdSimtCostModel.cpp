...
static bool readOptionalFitArray(const llvm::json::Object &parent,
                                 llvm::StringRef key,
                                 llvm::StringRef context,
                                 std::vector<double> &fit) {
  const auto *array = parent.getArray(key);
  if (!array)
    return true;
  if (array->size() != 4)
    return false;
  ...
  if (const auto *scalarMemory =
    ...
    profile.scalarIndirectDependencyLatencyCycles = resolveNumberOrMeasurement(
        *scalarMemory, "indirect_dependency_latency_system_cycles",
        ...
        reader, path);
    if (!readOptionalFitArray(*scalarMemory, "load_cycles_fit",
                              path + ".load_cycles_fit",
                              profile.scalarLoadCyclesFit) ||
        !readOptionalFitArray(*scalarMemory, "store_cycles_fit",
                              path + ".store_cycles_fit",
                              profile.scalarStoreCyclesFit)) {
      reader.setError(path + " optional scalar throughput fit is invalid");
    }
  }
