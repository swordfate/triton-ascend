...
analyzePlainOneDimensionalCumsum(Operation *op) {
  ...
  facts.elementType = getScalarTypeName(sourceType);
...
static std::string getAtomicOperation(Operation *op) {
  ...
  for (llvm::StringRef name :
       {"fadd", "umax", "umin", "xchg", "exch", "and", "xor", "add",
        "max", "min", "or"})
...
static bool isSupportedAtomicType(llvm::StringRef type,
  ...
  if (operation == "add" || operation == "fadd" || operation == "max" ||
    ...
    return type == "i32" || type == "i64" || type == "f16" ||
           type == "bf16" || type == "f32";
  return false;
}
...
analyzeAnchor(Operation *op, bool compileOn91095) {
  if (!op)
    return std::nullopt;
  ...
  if (name == "tt.gather") {
    ...
    descriptor.lowerability =
        simpleMixedLowerability("whole_module_pure_simt_requires_backend_check");
    ...
    facts.numBins = result && result.hasStaticShape() &&
                            result.getRank() == 1
                        ? result.getShape()[0]
                        : 0;
    ...
    descriptor.lowerability.allSimd =
        CandidateLoweringStatus::Unsupported;
    ...
    bool supported =
        input && result && input.hasStaticShape() && result.hasStaticShape() &&
        input.getRank() == 1 && result.getRank() == 1 &&
    ...
    descriptor.facts = *facts;
    descriptor.lowerability.allSimd =
        CandidateLoweringStatus::AliasesMixed;
    descriptor.lowerability.allSimdReasons.push_back(
    ...
    if (facts->axisExtent <= 0 ||
        !isSupportedCumsumType(facts->elementType)) {
      descriptor.lowerability.mixed =
          CandidateLoweringStatus::Unsupported;
      descriptor.lowerability.mixedReasons.push_back(
    ...
    if (op->getNumOperands() > 0) {
      ...
      if (offset) {
        ...
        if (auto offsetTensor =
                dyn_cast<RankedTensorType>(offset.getType()))
          facts.addressIsLaneVarying |=
              getStaticNumElements(offsetTensor) > 1;
      }
      facts.addressDependsOnLoadedIndex =
          pointerDependsOnLoadedIndex(op);
    }
    ...
    bool supported =
        result && result.hasStaticShape() && facts.updateElements > 0 &&
...
llvm::StringRef
mlir::ascend::stringifySimtAnchorKind(SimtAnchorKind kind) {
  switch (kind) {
...
llvm::StringRef mlir::ascend::stringifyCandidateLoweringStatus(
    CandidateLoweringStatus status) {
  switch (status) {
  case CandidateLoweringStatus::Unsupported:
    return "unsupported";
  case CandidateLoweringStatus::Native:
...
SimtAnchorPlan::materializableRoots() const {
...
SimtAnchorPlan
mlir::ascend::buildMixedSimtAnchorPlan(ModuleOp module,
                                       bool compileOn91095) {
  SimtAnchorPlan plan;
  ...
  if (plan.anchors.empty()) {
    ...
    plan.kernelLowerability.mixedReasons.push_back(
        "no_recognized_simt_anchor");
...
llvm::SmallVector<Operation *>
mlir::ascend::collectMixedSimtAnchors(ModuleOp module,
                                      bool compileOn91095) {
  return buildMixedSimtAnchorPlan(module, compileOn91095)
      .materializableRoots();
}
