...
static double mainScalarLoadCycles(double opCount, double uniqueLines,
  ...
  const double perLine =
      u <= profile.mainScalarLoadExtraLineHighThreshold
...
static double simtUniformLoadCycles(double opCount, bool sameLine,
                                    const StageModeProfile &profile) {
  const double k = std::max(1.0, opCount);
  const double marginal = sameLine
                              ? profile.simtUniformLoadSameLineSerialCycles
                              : profile.simtUniformLoadDiffLineIssueCycles;
  return profile.simtUniformLoadPrepCycles +
         profile.simtUniformLoadFillCycles + (k - 1.0) * marginal;
}
...
static double mte3StoreCycles(double opCount,
                              const StageModeProfile &profile) {
  const double k = std::max(1.0, opCount);
...
bool StageModeProfile::isValid(StageMode mode) const {
  ...
  const bool hasWhiteboxLoad =
      mode == StageMode::SIMD ? mainScalarLoadFillCycles > 0.0
  ...
  if (!std::all_of(
      ...
      !std::isfinite(mte3StoreSerialCycles) ||
      mte3StoreSerialCycles < 0.0 ||
      ...
      !std::all_of(scalarLoadCyclesFit.begin(), scalarLoadCyclesFit.end(),
                   [](double value) { return std::isfinite(value); }) ||
      !std::all_of(scalarStoreCyclesFit.begin(), scalarStoreCyclesFit.end(),
                   [](double value) { return std::isfinite(value); }) ||
