...
static void collectScalarPointerDependencySources(
    Operation *memoryOp, llvm::DenseSet<Operation *> &sources) {
  ...
  for (Operation *load : loads) {
    ...
    while (Operation *definition = base.getDefiningOp()) {
      if (definition->getName().getStringRef() != "tt.addptr" ||
        ...
        break;
      std::optional<int64_t> delta = getConstantInteger(definition->getOperand(1));
      if (!delta)
        return false;
      offset += *delta;
...
llvm::Error StageFeatureAnalysis::analyze(StagePartition &partition) const {
  ...
  for (LogicalStage &stage : partition.stages) {
    ...
    costModelDebug() << "features stage=" << stage.id << " loop=" << facts.hasLoop << " carried=" << facts.hasLoopCarriedDataDependency << " reduction=" << facts.hasReduction << " dot=" << facts.hasDot << " indirect=" << facts.hasIndirectMemory << " contiguous=" << facts.hasContiguousMemory << " conversion=" << facts.hasConversionPack << " scalar_share_line=" << facts.scalarLoadsShareLine << " store_share_line=" << facts.scalarStoresShareLine << "\n";
